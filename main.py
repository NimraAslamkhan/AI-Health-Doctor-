...
# load model===========================================
svc = pickle.load(open(r'C:\Users\123\PycharmProjects\Medical recomendation system\svc.pkl', 'rb'))
...
# ==========================helper funtions================
def helper(dis):
    # Ensure proper error handling and column existence checks
    # Description
    if 'Disease' in description.columns and 'Description' in description.columns:
        desc = description[description['Disease'] == dis]['Description']
        desc = " ".join(desc.values) if not desc.empty else "No description available"
    else:
        desc = "Description data is not available."

    # Precautions
    required_precaution_cols = ['Precaution_1', 'Precaution_2', 'Precaution_3', 'Precaution_4']
    if 'Disease' in precautions.columns and all(col in precautions.columns for col in required_precaution_cols):
        pre = precautions[precautions['Disease'] == dis][required_precaution_cols]
        pre = pre.values.tolist() if not pre.empty else ["No precautions available"]
    else:
        pre = ["Precaution data is not available."]

    # Medications
    if 'Disease' in medications.columns and 'Medication' in medications.columns:
        med = medications[medications['Disease'] == dis]['Medication']
        med = med.values.tolist() if not med.empty else ["No medications available"]
    else:
        med = ["Medication data is not available."]

    # Diets
    if 'Disease' in diets.columns and 'Diet' in diets.columns:
        die = diets[diets['Disease'] == dis]['Diet']
        die = die.values.tolist() if not die.empty else ["No diet recommendations available"]
    else:
        die = ["Diet data is not available."]

    # Workout
    if 'disease' in workout.columns and 'workout' in workout.columns:
        wrkout = workout[workout['disease'] == dis]['workout']
        wrkout = wrkout.values.tolist() if not wrkout.empty else ["No workout recommendations available"]
    else:
        wrkout = ["Workout data is not available."]
